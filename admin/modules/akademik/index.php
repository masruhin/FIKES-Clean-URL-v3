...
try {
  ...
  if (in_array($action, ['kalender', 'registrasi', 'penilaian'], true)) {
    if ($action === 'kalender') {
      ...
      $file = upload_doc('file_dokumen', $oldFile);
      $d = [trim($_POST['tahun_ajaran'] ?? ''), trim($_POST['kategori'] ?? ''), trim($_POST['judul'] ?? ''), $_POST['tanggal_mulai'] ?? '', $_POST['tanggal_selesai'] ?: null, trim($_POST['keterangan'] ?? ''), $file, (int)($_POST['nomor_urut'] ?? 1)];
      if ($d[0] === '' || $d[1] === '' || $d[2] === '' || $d[4] !== null && $d[4] < $d[3]) throw new Exception('Tahun ajaran, kategori, judul dan tanggal mulai wajib diisi. Tanggal selesai tidak boleh sebelum tanggal mulai.');
      if ($id) {
        $pdo->prepare('UPDATE akademik_kalender SET tahun_ajaran=?,kategori=?,judul=?,tanggal_mulai=?,tanggal_selesai=?,keterangan=?,file_dokumen=?,nomor_urut=? WHERE id=?')->execute([...$d, $id]);
        if ($oldFile && $file !== $oldFile) {
          $oldPath = $uploadDir . basename($oldFile);
          if (is_file($oldPath)) @unlink($oldPath);
        }
      } else {
        $pdo->prepare('INSERT INTO akademik_kalender(tahun_ajaran,kategori,judul,tanggal_mulai,tanggal_selesai,keterangan,file_dokumen,nomor_urut) VALUES(?,?,?,?,?,?,?,?)')->execute($d);
      }
  ...
  if ($action === 'delete') {
    ...
    if ($t) {
      ...
      if ($t === 'akademik_kalender') {
        $stFile = $pdo->prepare('SELECT file_dokumen FROM akademik_kalender WHERE id=? LIMIT 1');
        $stFile->execute([$deleteId]);
        $calendarFile = $stFile->fetchColumn();
      }
      $pdo->prepare("DELETE FROM $t WHERE id=?")->execute([$deleteId]);
      if ($t === 'akademik_kalender' && !empty($calendarFile)) {
        $calendarPath = $uploadDir . basename($calendarFile);
        if (is_file($calendarPath)) @unlink($calendarPath);
      }
...
<div class="ak-wrap">
  ...
  <div class="ak-grid">
    ...
    <section class="ak-panel<?= ($editType === 'kalender' && $editId) ? ' editing' : '' ?>" id="kalender">
      ...
      <form class="ak-form" method="post" enctype="multipart/form-data"><input type="hidden" name="action"
          value="kalender"><input type="hidden" name="id"
          value="<?= ($editType === 'kalender' ? ($editRow['id'] ?? 0) : 0) ?>">
        ...
        <div class="full"><label>Dokumen Kalender Akademik</label><input type="hidden" name="old_file"
            value="<?= e($editType === 'kalender' ? ($editRow['file_dokumen'] ?? '') : '') ?>"><input type="file"
            name="file_dokumen" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx"><small>PDF/Word/Excel/PowerPoint,
            maksimal 10 MB.
            <?= ($editType === 'kalender' && !empty($editRow['file_dokumen'])) ? 'File saat ini: <a href="../../uploads/akademik/' . rawurlencode(basename($editRow['file_dokumen'])) . '" target="_blank">' . e(basename($editRow['file_dokumen'])) . '</a>. Kosongkan jika tidak ingin mengganti.' : 'Opsional.' ?></small>
        </div>
      ...
      <div class="ak-table-wrap">
        <table class="ak-table">
          <tr>
            ...
            <td><?php if (!empty($x['file_dokumen'])): ?><a class="ak-btn light"
                href="../../uploads/akademik/<?= rawurlencode(basename($x['file_dokumen'])) ?>"
                target="_blank">Lihat</a><?php else: ?><span class="ak-muted">Tidak ada</span><?php endif; ?></td>
            <td>
